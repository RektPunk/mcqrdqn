...
class FQFAgent:
    ...
    def update(self, buffer: ReplayBuffer, batch_size: int):
        ...
        curr_dist = self.policy_net(states, taus_hat).gather(1, actions).squeeze(1)
        with torch.no_grad():
            _, next_taus_hat, next_tau_deltas = self.target_fpnet(next_states)
            next_policy_dist = self.policy_net(next_states, next_taus_hat)
            online_next_q = (next_policy_dist * next_tau_deltas.unsqueeze(1)).sum(dim=2)
            best_actions = (
                online_next_q.argmax(dim=1)
                .view(-1, 1, 1)
